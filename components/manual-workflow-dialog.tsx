...
const steps = [
  ...
  { id: 'step-3', title: 'Review & Save', description: 'Finalize your workflow' },
];
...
export function ManualWorkflowDialog() {
  ...
  const addNode = () => {
    const result = nodeSchema.safeParse({ currentNodeDescription, currentNodeAgent });
    if (!result.success) {
  ...
  const handleNextStep = () => {
    if (activeStep === 1) {
      const result = workflowDetailSchema.safeParse({ workflowName, workflowDescription });
      if (!result.success) {
        toast.error(result.error.errors[0].message);
        return;
      }
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-6xl h-[90vh] flex flex-col">
        ...
        <div className="flex justify-start mb-4">
          <Stepper value={activeStep} onValueChange={setActiveStep} className="w-full max-w-3xl">
            {steps.map((step, index) => (
              <StepperItem key={step.id} step={index + 1}>
                <StepperTrigger>
            ...
            {activeStep === 2 && (
              <div className="space-y-4">
                 <div>
                    ...
                    <div className="p-2 border rounded-md bg-muted min-h-[100px]">
                        {nodes.map(n => (
                          <div key={n.id} className="flex items-center justify-between text-sm p-1">
                            <span>{n.data.description} ({n.data.agent})</span>
                            <Button variant="ghost" size="icon" onClick={() => deleteNode(n.id)}>
             ...
             {activeStep === 3 && (
              <div>
                ...
                <p><strong>Description:</strong> {workflowDescription || 'N/A'}</p>
        ...
        <DialogFooter>
          {activeStep > 1 && <Button variant="outline" onClick={() => setActiveStep(s => s - 1)}>Back</Button>}
          {activeStep < steps.length && <Button onClick={handleNextStep}>Next</Button>}
          {activeStep === steps.length && <Button onClick={handleSaveWorkflow}>Save Workflow</Button>}
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
