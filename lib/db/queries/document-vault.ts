...
export async function getTempDocument(id: string, userId: string) {
  try {
    const [result] = await db.select()
      .from(tempDocuments)
      .where(and(
        eq(tempDocuments.id, id),
        eq(tempDocuments.userId, userId)
      ))
      .limit(1);
...
export async function deleteDocumentFromVault(documentId: string, userId: string) {
  try {
    // Get vector IDs before deletion for cleanup
    const chunks = await db.select({ vectorId: documentChunks.vectorId })
      .from(documentChunks)
      .innerJoin(documentVault, eq(documentChunks.documentId, documentVault.id))
      .where(and(
        eq(documentVault.id, documentId),
        eq(documentVault.userId, userId)
      ));

    // Delete document (chunks will be deleted by CASCADE)
    const [deletedDoc] = await db.delete(documentVault)
      .where(and(
        eq(documentVault.id, documentId),
        eq(documentVault.userId, userId)
      ))
...
export async function cleanupExpiredTempDocuments() {
  try {
    return await db.delete(tempDocuments)
